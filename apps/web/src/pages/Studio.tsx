        ...
        if (project && project.id === projectId) {
            ...
            if (outputs) {
                ...
                if (!hydratedOnce.current) {
                    ...
                    const nodeNames: Record<number, string> = { 1: 'Strategist', 2: 'Business Analyst', 3: 'Tech Lead', 4: 'Shipyard' }
                    bestByNode.forEach((output) => {
                        handleSSEEvent({
                            type: 'LOG',
                            nodeId: output.nodeId,
            ...
            if (project.deployment) {
                ...
                const stepFlags = [
                    { step: 'A' as const, done: dep.stepADone, label: 'Clone & inject schema' },
                    { step: 'B' as const, done: dep.stepBDone, label: 'Push to GitHub' },
                    { step: 'C' as const, done: dep.stepCDone, label: 'Deploy to DigitalOcean' },
            ...
            <header className="h-14 border-b border-border bg-bg-surface flex items-center justify-between px-4 shrink-0 z-20">
                ...
                <div className="flex items-center gap-2 absolute left-1/2 -translate-x-1/2">
                    ...
                    <button
                        ...
                        title={hasReachedShipyard ? 'Disabled after reaching Shipyard stage' : 'Retry entire pipeline from Node 1'}
                    >
                    ...
                    <button
                        ...
                        title={reviewReadyNode ? 'Approve current node and continue' : failedNode ? 'Retry failed node' : 'Start/Continue pipeline'}
                    >
