...
export const ShipyardNode = memo(
  ({ data }: { data: { status: NodeStatus } }) => {
    ...
    const doConsoleUrl = deployment?.doAppId
      ? `https://cloud.digitalocean.com/apps/${deployment.doAppId}`
      : null;
              ...
              <div className="flex flex-wrap gap-1.5">
                {deployment?.githubRepoUrl && (
                  <a
                    ...
                    target="_blank"
                    rel="noopener noreferrer"
                    className="flex items-center gap-1 text-[10px] font-semibold bg-bg-elevated border border-border rounded px-2 py-1 text-text-primary hover:border-accent-primary/50 transition-colors"
                    onClick={(e) => e.stopPropagation()}
                  >
                    <Github size={10} /> GitHub
                  </a>
