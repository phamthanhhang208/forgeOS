  ...
  return (
    <div className="bg-bg-elevated rounded-lg p-3 flex flex-col gap-1.5 border border-border/40">
      <div className="flex items-start justify-between gap-2">
        <span className="font-semibold text-[13px] text-text-primary leading-snug">
          {item.name}
        </span>
        {priority && (
          <span
            className={`text-[10px] font-bold px-2 py-0.5 rounded-full whitespace-nowrap shrink-0 ${PRIORITY_STYLES[priority] ?? ""}`}
          >
...
const PricingCard = ({ item }: { item: any }) => (
  <div className="bg-bg-elevated rounded-lg p-3 border border-border/40 flex flex-col gap-1">
    <div className="flex items-baseline justify-between">
      ...
      <span className="font-mono font-bold text-accent-primary text-[13px]">
        {item.price}
      </span>
    </div>
    {item.description && (
...
const PersonaCard = ({ item }: { item: any }) => (
  <div className="bg-bg-elevated rounded-lg p-3 border border-border/40 flex flex-col gap-2">
    ...
    {item.painPoints?.length > 0 && (
      <div>
        <p className="text-[11px] font-semibold text-accent-danger/80 uppercase tracking-wide mb-1">
          Pain Points
        </p>
        <ul className="flex flex-col gap-0.5">
          {item.painPoints.map((p: string, i: number) => (
            <li
              key={i}
              className="text-[12px] text-text-primary/80 flex gap-1.5"
            >
              <span className="text-accent-danger/60 shrink-0">•</span>
              {p}
            </li>
          ))}
        </ul>
    ...
    {item.goals?.length > 0 && (
      <div>
        <p className="text-[11px] font-semibold text-accent-success/80 uppercase tracking-wide mb-1">
          Goals
        </p>
        <ul className="flex flex-col gap-0.5">
          {item.goals.map((g: string, i: number) => (
            <li
              ...
              className="text-[12px] text-text-primary/80 flex gap-1.5"
            >
              <span className="text-accent-success/60 shrink-0">•</span>
              {g}
            </li>
          ))}
        </ul>
...
const StoryCard = ({ item }: { item: any }) => (
  <div className="bg-bg-elevated rounded-lg p-3 border border-border/40 flex flex-col gap-1.5">
    <p className="text-[13px] text-text-primary leading-relaxed">
      <span className="text-text-muted">As a </span>
      <span className="font-semibold">{item.asA}</span>
      <span className="text-text-muted">, I want to </span>
      <span className="font-medium">{item.iWantTo}</span>
      <span className="text-text-muted"> so that </span>
      <span className="font-medium">{item.soThat}</span>.
    </p>
...
const EndpointRow = ({ item }: { item: any }) => (
  <div className="flex items-start gap-2 py-1.5 border-b border-border/30 last:border-0">
    <span
      className={`text-[10px] font-mono font-bold px-1.5 py-0.5 rounded shrink-0 mt-0.5 ${HTTP_METHOD_STYLES[item.method] ?? "bg-border text-text-muted"}`}
    >
      {item.method}
    </span>
    <div className="flex flex-col gap-0.5">
      <code className="text-[12px] text-accent-primary font-mono">
        {item.path}
      </code>
...
const JsonViewer = ({
  ...
  if (data === null)
    return <span className="text-text-muted italic text-[12px]">—</span>;
  if (typeof data === "boolean")
    return (
      <span className="text-accent-warning font-mono text-[13px]">
        {data ? "Yes" : "No"}
      </span>
    );
  if (typeof data === "number") {
    if (parentKey === "estimatedDays")
      return (
        <span className="text-accent-primary font-mono text-[13px]">
          {data} day{data !== 1 ? "s" : ""}
        </span>
      );
    return (
      <span className="text-accent-primary font-mono text-[13px]">{data}</span>
    );
  }
  ...
  if (typeof data === "string") {
    ...
    if (parentKey === "priority" && PRIORITY_STYLES[data]) {
      return (
        <span
          className={`text-[10px] font-bold px-2 py-0.5 rounded-full ${PRIORITY_STYLES[data]}`}
        >
    ...
    if (parentKey === "method") {
      return (
        <span
          className={`text-[10px] font-mono font-bold px-1.5 py-0.5 rounded ${HTTP_METHOD_STYLES[data] ?? "bg-border text-text-muted"}`}
        >
          {data}
        </span>
      );
    ...
    if (data.length > 100)
      return (
        <p className="text-text-primary/90 leading-relaxed text-[13px] whitespace-pre-wrap">
          {data}
        </p>
      );
  ...
  if (Array.isArray(data)) {
    if (data.length === 0)
      return <span className="text-text-muted italic text-[12px]">None</span>;
      ...
      return (
        <div className="flex flex-col gap-1.5 mt-1">
          {data.map((item: any, i: number) => (
            <div
              key={i}
              className="flex items-baseline justify-between gap-2 py-1.5 border-b border-border/30 last:border-0"
            >
              <span className="text-[13px] text-text-primary/90">
                {item.feature}
              </span>
        ...
        return (
          <div className="flex flex-wrap gap-1.5 mt-1">
            {data.map((item, i) => (
              <span
                key={i}
                className="text-[11px] font-mono bg-bg-elevated border border-border/60 text-text-primary/80 px-2 py-0.5 rounded-md"
              >
                {item}
              </span>
            ))}
        ...
        <ul className="flex flex-col gap-1.5 mt-1">
          {data.map((item, i) => (
            <li key={i} className="text-[13px] text-text-primary/90 flex gap-2">
              <span className="text-accent-primary/50 shrink-0 mt-[3px]">
                ›
              </span>
    ...
    return (
      <div className="flex flex-col gap-3 mt-1">
        {data.map((item, i) => (
          <div
            key={i}
            className={`pt-2 ${i > 0 ? "border-t border-border/40" : ""}`}
          >
            <JsonViewer data={item} depth={depth + 1} parentKey={parentKey} />
          </div>
        ))}
      ...
      return (
        <div className="grid grid-cols-1 gap-2 mt-1">
          {entries.map(([key, value]) => (
            <div
              key={key}
              className="bg-bg-elevated rounded-lg p-2.5 border border-border/40"
            >
              ...
              <div className="flex flex-wrap gap-1">
                {(value as string[]).map((v, i) => (
                  <span
                    key={i}
                    className="text-[11px] bg-bg-base border border-border/60 text-text-primary/80 px-2 py-0.5 rounded-md font-mono"
                  >
                    {v}
                  </span>
                ))}
        ...
        {entries.map(([key, value]) => {
          ...
          return (
            <div
              ...
              className={
                isTopLevel
                  ? "flex flex-col gap-1.5 pb-4 border-b border-border/30 last:border-0"
                  : `flex ${isComplex ? "flex-col gap-1" : "gap-2 items-baseline"}`
              }
            >
              <span
  ...
  useEffect(() => {
    ...
    api
      ...
      .then((project) => {
        ...
        if (
          best?.jsonPayload &&
          typeof best.jsonPayload === "object" &&
          Object.keys(best.jsonPayload as object).length > 0
        ) {
      ...
      {isOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <motion.div
            ...
            className="relative w-full max-w-2xl bg-bg-surface border border-border rounded-2xl shadow-2xl flex flex-col max-h-[88vh] overflow-hidden"
          >
            {/* Header */}
            ...
            <div className="flex-1 min-h-0 overflow-y-auto p-5 flex flex-col gap-4">
              ...
              <div className="flex flex-col border border-border rounded-lg overflow-auto">
                {/* Tab bar */}
