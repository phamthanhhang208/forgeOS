...
/**
 * Ensure a repo has at least one commit so the Git Data API works.
 * First polls for auto_init to complete; if still empty, seeds a README.
 */
async function ensureRepoInit(octokit: Octokit, owner: string, repo: string): Promise<void> {
    // Quick poll — auto_init usually finishes within a few seconds
    for (let i = 0; i < 5; i++) {
        try {
    ...
    // Still empty — seed it ourselves via the Contents API (works on empty repos)
    console.log(`[GitHub] Repo ${owner}/${repo} still empty, seeding initial commit`)
...
export const github = {
    ...
    pushDirectory: async (localPath: string, owner: string, repo: string, branchName: string = 'main', settings?: Partial<AgencySettings>) => {
        ...
        await ensureRepoInit(octokit, owner, repo)
