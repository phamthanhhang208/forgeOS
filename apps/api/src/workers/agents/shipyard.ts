...
const log = (projectId: string, level: 'info' | 'warn' | 'error', message: string) => {
    console.log(`[Shipyard][${level.toUpperCase()}] ${message}`)
    return publishEvent(projectId, { type: 'LOG', nodeId: 4, level, message, timestamp: new Date().toISOString() })
}

const timed = async <T>(projectId: string, label: string, fn: () => Promise<T>): Promise<T> => {
...
export async function runShipyard(data: ShipyardJobData): Promise<Record<string, unknown>> {
    ...
    console.log(`[Shipyard] ▶ Starting shipyard for project ${projectId} (mode=${mode})`)
    await log(projectId, 'info', `Shipyard started (mode=${mode})`)

    // Fetch per-agency settings (fallback to env vars)
    const project = await prisma.project.findUnique({ where: { id: projectId }, select: { agencyId: true } })
    let settings: AgencySettings = {}
    if (project) {
        const agency = await prisma.agency.findUnique({ where: { id: project.agencyId }, select: { settings: true } })
        if (agency?.settings && typeof agency.settings === 'object') {
            settings = agency.settings as AgencySettings
        }
    }
    await log(projectId, 'info', `Using ${settings.githubOrg ? 'agency' : 'env'} settings`)
    ...
    if (!deployment.stepADone) {
        ...
        const boilerplateRepo = settings.goldenBoilerplateRepo || process.env.GOLDEN_BOILERPLATE_REPO!
        const boilerplateSHA = settings.goldenBoilerplateSha || process.env.GOLDEN_BOILERPLATE_SHA!
        ...
        await timed(projectId, 'Step A: checkout SHA + remove .git', async () => {
            ...
            await fs.promises.rm(path.join(localPath, '.git'), { recursive: true, force: true })
        })

        await timed(projectId, 'Step A: inject Prisma schema + .env.example', async () => {
            const schemaPath = path.join(localPath, 'prisma', 'schema.prisma')
            await fs.promises.mkdir(path.dirname(schemaPath), { recursive: true })
            const existing = await fs.promises.readFile(schemaPath, 'utf-8').catch(() => '')
    ...
    if (!deployment.stepBDone) {
        ...
        deployment = await prisma.deployment.update({
            where: { projectId },
            data: { stepBDone: true, githubRepoUrl, githubFullName, buildStatus: 'DEPLOYING' },
        })
        ...
        createLocalStack({
            ...
            .then(zipPath => prisma.deployment.update({ where: { projectId }, data: { zipPath } }))
            .catch(console.error)
    }
    ...
    if (!deployment.stepCDone && mode === 'NEW') {
        ...
        const app = await timed(projectId, 'Step C: DO App Platform createApp', () =>
            doClient.createApp({
                name: repoSlug,
    ...
    if (!deployment.stepDDone && deployment.doAppId && mode === 'NEW') {
        ...
        const liveUrl = await timed(projectId, 'Step D: poll DO waitForActive', () =>
...
Tags: ${[...output.techStack.frontend, ...output.techStack.backend].map(t => t.toLowerCase().replace(/[^a-z0-9]/g, '-')).join(', ')}`
        ...
        const region = settings.doSpacesRegion || (process.env.DO_SPACES_REGION ?? 'sgp1')
        const spaces = new S3Client({
            endpoint: `https://${region}.digitaloceanspaces.com`,
            region,
            credentials: {
                accessKeyId: settings.doSpacesKey || process.env.DO_SPACES_KEY!,
                secretAccessKey: settings.doSpacesSecret || process.env.DO_SPACES_SECRET!,
            },
        })
