...
const log = (projectId: string, nodeId: number, level: 'info' | 'warn' | 'error', message: string) =>
    publishEvent(projectId, { type: 'LOG', nodeId, level, message, timestamp: new Date().toISOString() })
import {
    DEMO_STRATEGIST,
    DEMO_ANALYST,
    DEMO_TECHLEAD,
...
const worker = new Worker<PipelineJobData>(
    ...
    async (job: Job<PipelineJobData>) => {
        ...
        console.log(`[Worker] 🔧 Received job for project ${projectId} node ${nodeId}`)
        await log(projectId, nodeId, 'info', `Processing started — ${NODE_LABELS[nodeId] ?? `Node ${nodeId}`}`)
        ...
        if (isDemoMode) {
            ...
            switch (nodeId) {
                ...
                case 4: {
                    ...
                    for (const step of ['A', 'B', 'C', 'D'] as const) {
                        await publishEvent(projectId, { type: 'SHIPYARD_STEP', step, status: 'START' })
                        await delay(DEMO_TIMING.shipyardStepMs)
                        await publishEvent(projectId, { type: 'SHIPYARD_STEP', step, status: 'DONE' })
                    }
                    ...
                    await publishEvent(projectId, { type: 'NODE_STATUS', nodeId, status: NodeStatus.APPROVED })
                    return
                }
                default:
            ...
            switch (nodeId) {
                case 1: {
                    await log(projectId, nodeId, 'info', 'Calling Strategist agent via Gradient API...')
                ...
                case 2: {
                    await log(projectId, nodeId, 'info', 'Calling Business Analyst agent...')
                    ...
                    await log(projectId, nodeId, 'info', 'Business Analyst response received')
                    break
                }
                case 3: {
                ...
                case 4: {
                    await log(projectId, nodeId, 'info', 'Starting Shipyard deployment...')
